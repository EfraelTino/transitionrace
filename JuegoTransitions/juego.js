...
if (windowWidth < windowHeight) {
    ...
    $myCanvas.style.width = 1000 + "px";//windowWidth+ "px";
    $myCanvas.style.height = 700 + "px";//45 + "em";
    $myCanvas.style.marginLeft = -70 + "%";
    $myCanvas.style.marginTop = 43 + "%";
    $cuadro.style.marginLeft = -9 + "em";//margin-left: 32em;  margin-top: 2%;
    $cuerpo.style.backgroundImage = "url(images/vertical.jpg)";
    $cuadro.style.marginTop = 2 + "em";
    $front.style.display = "none";
    $hudP.style.top = 10 + "px";
    $item.style.top = 550 + "px";
    $item.style.right = -20 + "%";
    $fin.style.backgroundImage = "url(images/overV.png)";
    $fin.style.marginTop=6+"%";
    $letras.style.fontSize=8+"px";
    $fin.style.zIndex=1;
    celu=true;
...
function recargaOIL(cantidad) {
    if (!llenadoOK) {
        if(gasolina<1){gasolina=10;}else{
...
async function update(dt) {
   ...
   // console.log(hud.puntos.value);
    ...
    if (gasolina < 1) { updateHud('combustible', Util.toInt(0, 0)) } else {
    ...
    updateHud('puntos', Util.toInt(puntaje * 10));
...
function canvas_read_touch(canvasA, e) {
   ...
   // console.log(touch.pageY + " / " + touch.pageX);
    if (touch.pageX < 250 && touch.pageY<350) { keyLeft = true; keyRight = false; }
    if (touch.pageX > 300 && touch.pageY<350) {  keyRight = true; keyLeft = false; }
    if (touch.pageY > 500 ) { keyFaster = false; }
...
function on_canvas_touch_end(e) {
    canvasA.tc_md = false;
    //console.log("centro"); 
    keyFaster = true; keyLeft = false; keyRight = false;
}
...
canvasA.addEventListener('mousedown', (e) => { on_canvas_mouse_down(e) }, false);
canvasA.addEventListener('mouseup', (e) => { on_canvas_mouse_up(e) }, false);
canvasA.addEventListener('mousemove', (e) => { on_canvas_mouse_move(e) }, false);
canvasA.addEventListener('touchstart', (e) => { on_canvas_touch_start(e) }, false);
canvasA.addEventListener('touchend', (e) => { on_canvas_touch_end(e) }, false);
canvasA.addEventListener('touchmove', (e) => { on_canvas_touch_move(e) }, false);
//-------------------------------------------------------------------------
...
function render() {
   ...
   if(celu){
    ctx.drawImage(img, width/4.5, 100);
...
Dom.on('resolution', 'change', function (ev) {
    ...
    switch (ev.target.options[ev.target.selectedIndex].value) {
        ...
        case 'high': w = 1280; h = 960; ratio = w / width; break;//w = 1024; h = 768;
