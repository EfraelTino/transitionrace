...
if ($token) {
    $sql_update = mysqli_query($dblink, "SELECT * FROM clientestra WHERE llabeglobal='$token'") or die(mysqli_error($link));
    ...
    if ($row > 0) {
        ...
        header("location: ../registroexpert.php?q=" . $encoded_query_string);
    } else {
        echo "<script>
